...
export default function SignupPage() {
  ...
  const passwordRequirements = useMemo<PasswordRequirement[]>(() => [
    { id: 'length', text: 'At least 7 characters', met: password.length >= 7 },
    { 
      id: 'numberSpecial', 
      text: 'At least 1 numbers and special character', 
      met: /(?=.*\d)(?=.*[!@#$%^&*(),.?":{}|<>])/.test(password) 
    },
    { id: 'uppercase', text: 'At least 1 uppercase character', met: /[A-Z]/.test(password) },
  ], [password]);
            ...
            <div className="space-y-3 transition-all duration-300 ease-in-out">
              ...
              <div className="space-y-2">
                {passwordRequirements.map((requirement) => (
                  <div key={requirement.id} className="flex items-center gap-3 transition-all duration-200">
                    <div className="shrink-0">
                      {requirement.met ? (
                        <svg className="w-5 h-5 text-orange-500 transition-colors duration-200" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                        </svg>
                      ) : (
                        <svg className="w-5 h-5 text-red-500 transition-colors duration-200" fill="currentColor" viewBox="0 0 20 20">
