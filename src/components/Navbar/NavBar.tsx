...
function NavBar() {
  ...
  return (
    <nav className="sticky top-0 z-50 flex items-center justify-between px-4 md:px-6 py-3 bg-white border-b border-gray-200 shadow-sm">
                      ...
                      {searchResults.map((result) => (
                        ...
                        >
                          <div className="mt-1 text-gray-400">
                            {result.type === 'subject' ? <BookIcon /> : <DocumentIcon />}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="font-medium text-gray-900 truncate">{result.title}</div>
                            {result.subjectName && (
                              <div className="text-sm text-gray-500 truncate">{result.subjectName}</div>
                            )}
                            {result.lastAccessed && (
      ...
      {/* Right: Actions */}
      <div className={`flex items-center gap-2 md:gap-4 ml-2 transition-all duration-300 ${isSearchOpen ? "opacity-0 scale-95 pointer-events-none" : "opacity-100 scale-100"}`}>
        <CreditsDisplay />
