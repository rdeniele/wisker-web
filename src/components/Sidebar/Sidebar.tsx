...
function Sidebar() {
  return (
    <aside
      className="group/sidebar sticky top-0 z-40 h-screen w-16 hover:w-56 bg-white dark:bg-gray-900 flex flex-col py-6 px-2 hover:px-4 border-r border-gray-200 dark:border-gray-800 transition-all duration-300 ease-in-out overflow-x-hidden"
    >
      {/* Logo */}
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="group-hover/sidebar:flex flex-col hidden">
          <div className="font-semibold text-gray-800 dark:text-gray-100">Student</div>
          <div className="text-xs text-gray-500 dark:text-gray-400">Wisker User</div>
        </div>
      </div>
      ...
      <nav className="flex-1 flex flex-col gap-2">
        {navLinks.map((link) => (
          ...
          >
            ...
            <span className="ml-2 truncate group-hover/sidebar:inline-block hidden transition-all duration-200">{link.name}</span>
          </a>
        ))}
      </nav>
    </aside>
