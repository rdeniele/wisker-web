...
async function testDynamicPricing() {
  ...
  try {
    ...
    activePlans.forEach(plan => {
      console.log(`   - ${plan.displayName}: ₱${plan.monthlyPrice}/mo, ₱${plan.yearlyPrice}/year`);
    });
    console.log();
    ...
    console.log(`   Price range: ₱${Math.min(...activePlans.map(p => p.monthlyPrice))} - ₱${Math.max(...activePlans.map(p => p.monthlyPrice))}/mo`);
    console.log(`   Credit range: ${Math.min(...activePlans.map(p => p.dailyCredits))} - ${Math.max(...activePlans.map(p => p.dailyCredits))} daily`);
    console.log();
